...
const initialData = [
    { category: 'Lighting', items: [{ name: 'Motion Sensor', quantity: 0, unitPrice: 1500 }] },
    {
        category: 'Blinds',
        items: [
            { name: 'Track', quantity: 0, unitPrice: 300 },
            { name: 'Motor', quantity: 0, unitPrice: 10000 },
        ]
    },
    {
        category: 'Curtains',
        items: [
            { name: 'Track', quantity: 0, unitPrice: 800 },
            { name: 'Motor', quantity: 0, unitPrice: 10000 },
        ]
    },
...
const Qproducts = ({ onSubmit, totalProjectCost }) => {
    ...
    const handleChange = (catIndex, itemIndex, field, value) => {
        ...
        const val = field === 'quantity' ? Math.max(0, parseFloat(value) || 0) : value;
        updatedData[catIndex].items[itemIndex][field] = val;
        setData(updatedData);
    };
    ...
    const handleSubmit = () => {
        ...
        data.forEach((category, catIndex) => {
            ...
            const shortCatKey = category.category.split(' ')[0].replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
            categoryTotals[`total${shortCatKey}Cost`] = subtotal;

            category.items.forEach((item, itemIndex) => {
                if (item.quantity > 0) {
                    const shortItemKey = item.name.split(' ')[0].replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
                    const prefix = `${shortCatKey}${shortItemKey}`;

                    selectedItems[`${prefix}Qty`] = item.quantity;
                    selectedItems[`${prefix}Price`] = item.unitPrice;
    ...
    return (
        <div className="quotation-container">
            ...
            <div className="summary-section">
                <h3>Total Products Cost: ₹ {totalProductsCost.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</h3>
                <h3>Total (Before Installation Charges): ₹ {allTotalWithoutCharges.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</h3>
